...
| <a name="AUTH_METHOD">`AUTH_METHOD`</a> | Second-factor authentication method. IBKR currently **mandates passkey**, so this defaults to `passkey`. The supported values are `passkey` and `totp` (the legacy Mobile Authenticator App). The two are **mutually exclusive**. When `passkey`, IBGA automatically clicks the passkey "Authenticate" button; the passkey itself is served by the separate [soft-fido2](https://github.com/huieric/soft-fido2) container over USB/IP (see [Unattended Passkey Login](../faq.md#how-to-setup-unattended-passkey-software-security-key-login)). When `totp`, IBGA generates and enters the 6-digit code using `TOTP_KEY`. |
| <a name="TOTP_KEY">`TOTP_KEY`</a> | Legacy [TOTP (Mobile Authenticator App) login](../faq.md#how-to-setup-totp-mobile-authenticator-app-automated-login) secret. Only consulted when `AUTH_METHOD=totp`. IBKR no longer offers this as a new authentication option. |
